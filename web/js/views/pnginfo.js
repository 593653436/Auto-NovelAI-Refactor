...
function renderChat(body) {
  ...
  const hint = el("div", { class: "muted", style: "font-size:12px;margin-top:6px;", text: "📝 无图=tag提取(Qwen3.8-27B 云端4090·无审查·流式·保留思考)；🎞 有图=图描述(用下方模型)。例：提取场景/动作" });
  ...
  const modelSel = el("select", { style: "max-width:290px;", value: "Gliese-Qwen3.5-9B-Abliterated-Caption.Q4_K_M.gguf" }, [
    el("option", { value: "Gliese-Qwen3.5-9B-Abliterated-Caption.Q4_K_M.gguf", text: "🎞 Gliese caption (图像描述)" }),
    el("option", { value: "qwen3.8-27b-cloud", text: "🚀 Qwen3.8-27B (云端4090 提取)" }),
  ]);
